...
const baseRegex = "\\s*[@#]\\s*sourceMappingURL\\s*=\\s*([^\\s]*)(?![\\S\\s]*sourceMappingURL)";
// Matches /* ... */ comments
...
async function readSourceMap(url) {
    ...
    if (dataUrlMatch) {
        ...
        try {
            ...
            return [JSON.parse(jsonSourceMap), this.context];
        }
        catch (ex) {
            let shortenedSourceMap = base64SourceMap.substr(0, 50);
            throw new Error(`Cannot parse inline SourceMap '${shortenedSourceMap}': ${ex}`);
        }
    }
    else {
...
async function loadSourceMap(sourceMapUrl, { keepRelativeSources }) {
    const [sourceMap, sourcesContext] = await readSourceMap.call(this, sourceMapUrl);
    ...
    const sourcesPromises = sources.map(async (source, sourceIndex) => {
        let absolutePath;
        try {
            absolutePath = resolveAbsolutePath(sourcesContext, source);
        }
        catch (ex) {
            this.emitWarning(new Error(`Cannot find source file '${source}': ${ex}`));
            return {
                source: source,
                content: sourcesContent[sourceIndex] !== null
        ...
        try {
            ...
            this.emitWarning(new Error(`Cannot open source file '${absolutePath}': ${ex}`));
            return {
                source: absolutePath,
                content: null
            };
...
const loader = function (rawInputSource, rawInputSourceMap) {
    ...
    const inputSource = typeof rawInputSource === "string"
        ? rawInputSource
